...
import { getARStylingOptions } from "./ar";
...
async function loadNodeDeps() {
  ...
  if (typeof ctor !== "function") throw new Error("Nao foi possivel carregar qr-code-styling no Node.js.");

  const nodeCanvas = (canvasMod as { default?: unknown }).default ?? canvasMod;
  const JSDOM = (jsdomMod as { JSDOM?: unknown }).JSDOM;
  if (!JSDOM) throw new Error("Dependencia jsdom nao encontrada. Instale: npm i jsdom");

  return { QRCodeStyling: ctor as QRCodeStylingCtor, nodeCanvas, JSDOM };
}

async function toBuffer(value: Buffer | Blob | null): Promise<Buffer> {
  if (!value) throw new Error("Falha ao gerar QR Code.");
...
export async function generateQRCodeBuffer(input: GenerateQRCodeNodeInput): Promise<Buffer> {
  ...
  const styleInput = input.style ?? input.appearance;
  const baseOptions = input.arProfile
    ? getARStylingOptions(payload, styleInput, input.arProfile)
    : getStylingOptions(payload, styleInput);

  const qrCode = new QRCodeStyling({ ...baseOptions, nodeCanvas, jsdom: JSDOM });
